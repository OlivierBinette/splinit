...
#'
#' @usage splinit(Y, param=NULL, knn = ceiling(nrow(Y)^0.33) + 1, rep = 20, ...)
#'
#' @param Y matrix of data points. Each row must represent an observation.
#' @param param Circular parameterization of the data points by angles in the interval [0, 2 pi). NULL by default, in which case the parametrization is estimated.
#' @param knn Number or neighbors to consider in the isomap-type intrinsic distance approximation. Used only for unknown parameterizations (param=NULL).
#' @param rep Number of repetitions to use for the minimal hamiltonian cycle approximation. Increase for greater accuracy and stability. Used only for unknown parameterizations (param=NULL).
#' @return A matrix containing the evaluation of the splines at the points specified by eval.pts.
#'
#' @example
# angles = 2*pi*(1:20)/20
...
#'
#' @note Use only with more than 30 observations (cross validation may
#'     fail ortherwise). Stability of the estimate can be improved by increasing the
#'     _rep_ parameter.
#'
#' @import glmnet MASS tspmeta vegan TSP
#' @export
